...
public class DeleteBookingServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<BookingRecordBean> delrec = (ArrayList<BookingRecordBean>) request.getSession().getAttribute("delRec");
		...
		for(BookingRecordBean b : delrec) {
			new BookingRecordDAOImpl(services).deleteByRecordId(b.getId());
		}
		...
		if(records.size()==0){
			...
			request.getRequestDispatcher("/CustomerBookingServlet?bookingid="+booking.getId()).forward(request, response);
		}
		else{
		try {
